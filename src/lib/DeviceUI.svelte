...
<div class="device-ui-container">
    <div class="device-header">
        <h1>{currentTime}</h1>
        <h3>
            <span class="trashday-countdown">{daysToTrashday}</span> days to trash day
        </h3>
    </div>
    ...
    <div class="device-main" style="background-color: {backgroundColor}">
        ...
        <Popup message={notificationMessage} isOpen={isLatestChangeOpen} closePopup={closeLatestChangePopup} />
    
        <!-- Full Notification Log Popup (also centered inside device-main) -->
        <Popup notificationLog={notificationLog} isOpen={isNotificationOpen} closePopup={closeNotificationPopup} />
            ...
            <div class="footer-row">
                ...
                <div class="button-column">
                    <div class="button-container">
                        <RoundButton icon={NotificationIcon} on:click={handleNotificationClick} />
                        <RoundButton icon={GearIcon} on:click={handleSettingsClick} />
                        <RoundButton icon={ChartsIcon} on:click={handleGraphClick} />
                    </div>
                </div>
            </div>
        </div>
